# Simulation of A177 Stockton Road junction
...
class TrafficLightController:
    def __init__(self):
        self.time = 0
        self.phases = [ # combinations for traffic lights - the cycle
        ...
        self.current_phase = 0 # phase starts at time 0
        self.phase_timer = 0

    def update(self):
        self.time += 1
        self.phase_timer += 1
        if self.phase_timer >= self.phases[self.current_phase]['duration']: # change phase once duration over
            self.phase_timer = 0 # reset timer
            self.current_phase = (self.current_phase + 1) % len(self.phases) #  go to next phase

    def get_colors(self):
        return self.phases[self.current_phase]['ns'], self.phases[self.current_phase]['ew'] # change light colour
...
total_departed = 0 # all vehicles that have completed journey
cumulative_journey_time = 0 # total frames from spawn to departure

# parameters
spawn_rate = 0.25
car_spacing = 0.70 # the look ahead distance
min_gap = 0.55 # emergency braking distance
target_speed_val = 0.18 # 0.18 units per frame

# ROUTING
routes = { 
    'W_Straight': {'start_x': -9, 'target_y': 1.5, 'start_y': 1, 'dx': 1,  'dy': 0,  'stop_line': -2.3, 'light': 'ew', 'type': 'straight', 'split_start_x': -5.5, 'split_end_x': -3.5}, ###
    'E_Straight': {'start_x': 9,  'target_y': -1.5, 'start_y': -1, 'dx': -1, 'dy': 0,  'stop_line': 2.3,  'light': 'ew', 'type': 'straight', 'split_start_x': 5.5, 'split_end_x': 3.5}, ###
    'N_Straight': {'start_x': 1, 'target_x': 1.5, 'start_y': 9, 'dx': 0, 'dy': -1, 'stop_line': 3.3, 'light': 'ns', 'type': 'straight', 'shift_x': 1, 'split_start_y': 8.5, 'split_end_y': 6},
    'S_Straight': {'start_x': -1, 'target_x': -1.5, 'start_y': -9, 'dx': 0, 'dy': 1, 'stop_line': -3.3, 'light': 'ns', 'type': 'straight', 'shift_x': -1, 'split_start_y': -8.5, 'split_end_y': -6},

    'W_Left': {'start_x': -9, 'start_y': 2.5, 'dx': 1, 'dy': 0, 'stop_line': -2.3, 'light': 'ew', 'type': 'left',
               'turn_start': -2, 'cx': -2, 'cy': 3, 'rx': 1, 'ry': 0.5, 'start_angle': -np.pi/2, 'end_angle': 0, 'end_dx': 0, 'end_dy': 1},
    'E_Left': {'start_x': 9, 'start_y': -2.5, 'dx': -1, 'dy': 0, 'stop_line': 2.3, 'light': 'ew', 'type': 'left',
               'turn_start': 2, 'cx': 2, 'cy': -3, 'rx': 1, 'ry': 0.5, 'start_angle': np.pi/2, 'end_angle': np.pi, 'end_dx': 0, 'end_dy': -1},
    'N_Left': {'start_x': 1, 'target_x': 1.5, 'start_y': 9, 'dx': 0, 'dy': -1, 'stop_line': 3.3, 'light': 'ns', 'type': 'left',
               'turn_start': 3, 'cx': 2, 'cy': 3, 'rx': 0.5, 'ry': 1.5, 'start_angle': np.pi, 'end_angle': 3*np.pi/2, 'end_dx': 1, 'end_dy': 0, 'split_start_y': 8.5, 'split_end_y': 6},
    'S_Left': {'start_x': -1, 'target_x': -1.5, 'start_y': -9, 'dx': 0, 'dy': 1, 'stop_line': -3.3, 'light': 'ns', 'type': 'left',
               'turn_start': -3, 'cx': -2, 'cy': -3, 'rx': 0.5, 'ry': 1.5, 'start_angle': 0, 'end_angle': np.pi/2, 'end_dx': -1, 'end_dy': 0, 'split_start_y': -8.5, 'split_end_y': -6},

    'W_Right': {'start_x': -9, 'target_y': 0.5, 'start_y': 1, 'dx': 1, 'dy': 0, 'stop_line': -2.3, 'light': 'ew', 'type': 'right',
                'turn_start': -1.5, 'cx': -1.5, 'cy': -5.5, 'rx': 2.5, 'ry': 6, 'start_angle': np.pi/2, 'end_angle': 0, 'end_dx': 0, 'end_dy': -1, 'split_start_x': -5.5, 'split_end_x': -3.5}, ###
    'E_Right': {'start_x': 9, 'target_y': -0.5, 'start_y': -1, 'dx': -1, 'dy': 0, 'stop_line': 2.3, 'light': 'ew', 'type': 'right',
                'turn_start': 1.5, 'cx': 1.5, 'cy': 5.5, 'rx': 2.5, 'ry': 6, 'start_angle': 3*np.pi/2, 'end_angle': np.pi, 'end_dx': 0, 'end_dy': 1, 'split_start_x': 5.5, 'split_end_x': 3.5}, ###
    'N_Right': {'start_x': 1, 'target_x': 0.5, 'start_y': 9, 'dx': 0, 'dy': -1, 'stop_line': 3.3, 'light': 'ns', 'type': 'right',
                'turn_start': 1.5, 'cx': -5.5, 'cy': 1.5, 'rx': 6, 'ry': 3, 'start_angle': 0, 'end_angle': -np.pi/2, 'end_dx': -1, 'end_dy': 0, 'split_start_y': 8.5, 'split_end_y': 6},
    'S_Right': {'start_x': -1, 'target_x': -0.5, 'start_y': -9, 'dx': 0, 'dy': 1, 'stop_line': -3.3, 'light': 'ns', 'type': 'right',
                'turn_start': -1.5, 'cx': 5.5, 'cy': -1.5, 'rx': 6, 'ry': 3, 'start_angle': np.pi, 'end_angle': np.pi/2, 'end_dx': 1, 'end_dy': 0, 'split_start_y': -8.5, 'split_end_y': -6},
}
...
def draw_intersection(): # draw road
    ...
    center_color, center_lw, center_dash = 'black', 3, (0, (6, 6)) # separate opposite direction traffic dashed line
    lane_color, lane_lw, lane_dash = 'gray', 1.5, (0, (5, 5)) # different lane divider
    arrow_color = 'darkgray'

    # draw it
    for p in [[[-9, -2], [3, 3]], [[-2, -2], [3, 9]], [[2, 9], [3, 3]], [[2, 2], [3, 9]],
              [[-9, -2], [-3, -3]], [[-2, -2], [-3, -9]], [[2, 9], [-3, -3]], [[2, 2], [-3, -9]]]:
        ax.plot(p[0], p[1], color=edge_color, lw=edge_lw)
    ax.plot([-2, -9], [0, 0], color=center_color, lw=center_lw, linestyle=center_dash); ax.plot([2, 9], [0, 0], color=center_color, lw=center_lw, linestyle=center_dash)   
    ax.plot([0, 0], [3, 9], color=center_color, lw=center_lw, linestyle=center_dash); ax.plot([0, 0], [-3, -9], color=center_color, lw=center_lw, linestyle=center_dash) 
    
    ax.plot([-2, -9], [2, 2], color=lane_color, lw=lane_lw, linestyle=lane_dash); ax.plot([-2, -4.5], [1, 1], color=lane_color, lw=lane_lw, linestyle=lane_dash) 
    ax.plot([2, 9], [-2, -2], color=lane_color, lw=lane_lw, linestyle=lane_dash); ax.plot([2, 4.5], [-1, -1], color=lane_color, lw=lane_lw, linestyle=lane_dash) 
    ax.plot([1, 1], [3, 6.2], color=lane_color, lw=lane_lw, linestyle=lane_dash); ax.plot([-1, -1], [-3, -6.2], color=lane_color, lw=lane_lw, linestyle=lane_dash)
    ...
    draw_turn(-8.2, 2.5, 0.8, 0, 0, 0.4); draw_arrow(-8.2, 1, 1.2, 0); ax.plot([-7.7, -7.7], [1, 0.6], color=arrow_color, lw=1.5); draw_arrow(-7.7, 0.6, 0, -0.3)          
    draw_turn(-3.5, 2.5, 0.8, 0, 0, 0.4); draw_arrow(-3.5, 1.5, 1.2, 0); draw_turn(-3.5, 0.5, 0.8, 0, 0, -0.4)   
    draw_arrow(8.2, -1, -1.2, 0); ax.plot([7.7, 7.7], [-1, -0.6], color=arrow_color, lw=1.5); draw_arrow(7.7, -0.6, 0, 0.3)            
    draw_turn(8.2, -2.5, -0.8, 0, 0, -0.4); draw_turn(3.5, -0.5, -0.8, 0, 0, 0.4); draw_arrow(3.5, -1.5, -1.2, 0); draw_turn(3.5, -2.5, -0.8, 0, 0, -0.4)  
    draw_turn(0.5, 5.5, 0, -0.8, -0.4, 0); draw_arrow(1.5, 5.5, 0, -1.2); ax.plot([1.5, 1.7], [5, 5.0], color=arrow_color, lw=1.5); draw_arrow(1.7, 5, 0.2, 0)                                        
    draw_turn(-0.5, -5.5, 0, 0.8, 0.4, 0); draw_arrow(-1.5, -5.5, 0, 1.2); ax.plot([-1.5, -1.7], [-5, -5.0], color=arrow_color, lw=1.5); draw_arrow(-1.7, -5, -0.2, 0) 

    # ROAD LABELS
    text_props = dict(fontsize=11, fontweight='bold', color='darkslategray', va='center', ha='center', bbox=dict(facecolor='white', edgecolor='none', alpha=0.7, pad=1)) # properties for text
    ...
    for v in vehicles:
                            ...
                            for blocker in vehicles:
                                ...
                                if same_origin and blocker['id'] < other['id']: # blocker is ahead of other 
                                    ...
                                    if l_diff < 0.6: # then in same lane
                                        if b_route['type'] == 'right' and blocker['state'] in ['approaching', 'turning']: # blocker is stuck
                                            is_blocked = True
        ...
        for other in vehicles:
            ...
            if dist < vision_length: # if other car close enough
                ...
                if ahead_dist > 0.1 and lat_dist < 0.45: # must be ahead and in the same lane
                    heading_dir = v['hx'] * other['hx'] + v['hy'] * other['hy']
                    if heading_dir > 0.5:  # facing the same way? yes=1, no=-1
        ...
        if v['state'] == 'approaching':
            ...
            if route['type'] == 'straight' and 'shift_x' in route: # going straight but middles of lane do not align
                if (route['dy'] < 0 and v['y'] <= 3.0) or (route['dy'] > 0 and v['y'] >= -3.0):
                    v['state'] = 'crossing'
            ...
            v['y'] += v['speed'] * route['dy']
            if route['dy'] < 0:
                progress = (3.0 - v['y']) / 6.0 # how far through the intersection
            else:
                progress = (v['y'] - (-3.0)) / 6.0
            progress = np.clip(progress, 0, 1)
            
            start_pos_x = route.get('target_x', route['start_x']) # how far to slide horizontally to be in middle of lane
            v['x'] = start_pos_x + (route['shift_x'] - start_pos_x) * (0.5 - 0.5 * np.cos(np.pi * progress))
            if progress >= 1:
                v['state'] = 'departing'; v['x'] = route['shift_x'] # has crossed intersection

        elif v['state'] == 'turning':
            omega = v['speed'] / np.sqrt((route['rx'] * np.sin(v['angle']))**2 + (route['ry'] * np.cos(v['angle']))**2) # angular vel - how much turning in one frame
