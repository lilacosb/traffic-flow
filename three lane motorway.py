# Simulation of a three-lane motorway. 
...
# random braking
high_speed_threshold = 65 # speeds above this are susceptible to random distracted braking
...
def update(frame):
    ...
    raw_wave = 5000 + 3000 * np.sin(cycle * 2 * np.pi) 
    q_current = np.clip(raw_wave, a_min=1000, a_max=8000) # cap lower values so not negative
    ...
    for v in vehicles:
        ...
        if same_lane_ahead:
            ...
            if distance_ahead < (lookahead_dist * 0.7) and v['desired_speed'] > (car_ahead['speed'] + overtake_speed_advantage):
                ...
                if v['lane'] < 3 and can_change_lane: # able to overtake?
        ...
        if not wants_to_overtake:
            if catching_up:
                if v['signal_dir'] == -1: v['signal_dir'] = 0 
            else:
                if v['signal_dir'] == 1: 
                    v['signal_dir'] = 0 
    ...
    if np.random.rand() < prob_arrival:
        ...
        if valid_lanes:
            weights = {1: 0.7, 2: 0.2, 3: 0.1} # likelihoods of spawning in each lane
    ...
    # ACCELERATION AND SPEED BY INTERACTION
    jam_gap = 0.006 
    for v in vehicles:
        max_safe_speed = v['desired_speed']
        
        if v['lane'] == 3: # go faster when pressure behind
            ...
            if same_lane_behind:
                ...
                if distance_behind < (lookahead_dist * 1.5):
                    speed_boost = max(v['desired_speed'] + 2, car_behind['speed'] + 2)
        ...
        if v['lane'] < 3: # can undertake if in congestion
        ...
        if v['signal_dir'] != 0:
            ...
            for other in target_lane_cars:
                ...
                if gap < min_physical_gap: # fall back if car directly alongside
                    if other['y'] >= v['y']: max_safe_speed = min(max_safe_speed, max(0, v['speed'] - 5))
                    continue
                if other['y'] >= v['y']:
                    time_gap = gap / (max(v['speed'], 1) / 3600)
    ...
    if current_time > 0.5:
        ...
        alpha = 0.15 # data points made up of 20% current reality, 80% historicla memory - prevents big spikes from frame to frame
    ...
    if history_time:
        ax_speed.set_xlim(0.5, max(60, history_time[-1] + 5)) # continuous scrolling
...
# flow line
ax_flow = ax_speed.twinx() # second y axis scale
line_flow, = ax_flow.plot([], [], lw=3, color='black', alpha=0.3, label='Live flow') # alpha transparency
ax_flow.set_ylabel('Live flow (veh/hr)')
ax_flow.set_ylim(0, 9000) 
...
ani = animation.FuncAnimation(fig, update, interval=50, blit=False, cache_frame_data=False) # creates abunatuib
plt.tight_layout()
plt.show()